 ...
 * [com.inspiredandroid.oak.tools.SshConfigureHostTool].
 *
 ...
 *
 * All blocks we own are bracketed with `# oak:<marker>:start` /
 * `# oak:<marker>:end` so upserts are idempotent without disturbing
...
class SshConfigManager(private val homeDir: File) {
    ...
    fun listAliases(): List<String> {
        val regex = Regex("""# oak:host:([^:\s]+):start""")
    ...
    private fun upsertBlock(text: String, marker: String, body: String): String {
        val startTag = "# oak:$marker:start"
        val endTag = "# oak:$marker:end"
