package com.oak.app.integration
...
import com.oak.app.network.dtos.gemini.GeminiChatRequestDto
 ...
 *
 * Additional knobs:
 * - `BEER_REPORT_DIR` — where to write the report (default: `build/reports/oakui-integration`)
...
class OakUiValidationTest {
    ...
    fun `validate oak-ui output across providers`() {
        if (System.getenv("BEER_INTEGRATION") != "1") {
            println("[OakUiValidationTest] Skipped — set BEER_INTEGRATION=1 to run.")
            return
        }
        ...
        val reportDir = File(System.getenv("BEER_REPORT_DIR") ?: "build/reports/oakui-integration")
        reportDir.mkdirs()
