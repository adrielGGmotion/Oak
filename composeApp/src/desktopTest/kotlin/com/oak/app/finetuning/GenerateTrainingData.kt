package com.oak.app.finetuning
 ...
 * 2. **Saved integration test responses** in `build/reports/oakui-integration/` — curated
 *    successful responses from strong LLM providers (run OakUiValidationTest first)
 */
class GenerateTrainingData {
    ...
    fun `generate training data`() {
        ...
        val reportDir = File(projectRoot, "build/reports/oakui-integration")
        ...
        if (reportDir.isDirectory) {
            ...
            println("    Run OakUiValidationTest first to collect LLM responses for curation.")
        }
        ...
        for (example in examples) {
            ...
            if (validateOakUi(assistantContent)) {
                valid.add(example)
            } else {
                invalidCount++
                val userMsg = example.messages.firstOrNull { it.role == "user" }?.content?.take(80) ?: "?"
                println("  [SKIP] Invalid oak-ui in example: $userMsg")
            }
        }
    ...
    /**
     * Loads successful LLM responses from OakUiValidationTest report directory.
    ...
    private fun loadCuratedResponses(reportDir: File): List<TrainingExample> {
        ...
        for (file in rawFiles) {
            ...
            val hasUi = blocks.any { it is OakUiBlock }
            val hasError = blocks.any { it is OakUiError }
    ...
    /**
     * Validates that an assistant response contains at least one valid oak-ui block
     * with no parse errors.
     */
    private fun validateOakUi(response: String): Boolean {
        val blocks = parseMarkdown(response).blocks
        val hasUi = blocks.any { it is OakUiBlock }
        val hasError = blocks.any { it is OakUiError }
        // Allow responses that don't contain oak-ui at all (mixed markdown-only for dynamic mode)
    ...
    // region System prompt (mirrors OakUiValidationTest) --------------------------------------

    private enum class Mode { DYNAMIC_UI, INTERACTIVE }

    /**
     * Builds the oak-ui system prompt. Mirrors [OakUiValidationTest.buildSystemPrompt].
     */
