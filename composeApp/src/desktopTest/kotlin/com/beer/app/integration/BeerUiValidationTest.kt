 ...
 * 4. When green, copy the now-passing payloads into `BeerUiParserTest` as regressions
 */
class BeerUiValidationTest {
    ...
    fun `validate beer-ui output across providers`() {
        ...
        for (provider in providers) {
            println("\n[BeerUiValidationTest] === ${provider.label} (${provider.modelId}) ===")
    ...
    fun `replay saved responses`() {
        ...
        println("\n[BeerUiValidationTest] Replaying ${rawFiles.size} saved responses from ${reportDir.absolutePath}")
        var success = 0
        var partial = 0
        var noUi = 0
        var parseError = 0
        ...
        for (file in rawFiles) {
            ...
            val ui = blocks.filterIsInstance<BeerUiBlock>().size
            val err = blocks.filterIsInstance<BeerUiError>().size
    ...
    private fun runPrompt(
        ...
        val errorBlocks = blocks.filterIsInstance<BeerUiError>()
    ...
    private fun writeReport(reportDir: File, results: List<PromptResult>, providers: List<ProviderSpec>) {
        val report = buildString {
            appendLine("# beer-ui Integration Report")
            appendLine()
        ...
        println("\n[BeerUiValidationTest] Report written to: ${File(reportDir, "report.md").absolutePath}")
    }

    private fun printSummary(results: List<PromptResult>, providers: List<ProviderSpec>) {
        println("\n[BeerUiValidationTest] === SUMMARY ===")
        for (p in providers) {
            val r = results.filter { it.provider.slug == p.slug }
    ...
    /**
     * Duplicates the beer-ui portion of the app's system prompt. Mirrors
    ...
    companion object {
        private val TEST_PROMPTS = listOf(
            TestPrompt(
                "simple-question",
                "Simple question expecting a text response with beer-ui elements",
            ...
            TestPrompt(
                "mixed-markdown",
                "Response that mixes markdown explanation with beer-ui interactive block",
