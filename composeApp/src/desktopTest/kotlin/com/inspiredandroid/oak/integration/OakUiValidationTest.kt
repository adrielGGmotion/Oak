package com.inspiredandroid.oak.integration

import com.inspiredandroid.oak.data.Service
import com.inspiredandroid.oak.network.Requests
import com.inspiredandroid.oak.network.ServiceCredentials
import com.inspiredandroid.oak.network.dtos.anthropic.AnthropicChatRequestDto
import com.inspiredandroid.oak.network.dtos.anthropic.extractText
import com.inspiredandroid.oak.network.dtos.gemini.GeminiChatRequestDto
import com.inspiredandroid.oak.network.dtos.gemini.extractText
import com.inspiredandroid.oak.network.dtos.openaicompatible.OpenAICompatibleChatRequestDto
import com.inspiredandroid.oak.ui.markdown.OakUiBlock
import com.inspiredandroid.oak.ui.markdown.OakUiError
import com.inspiredandroid.oak.ui.markdown.parseMarkdown
 ...
 * validates that their responses contain well-formed oak-ui blocks.
 *
 * By default this test is **skipped** — it only runs when `OAK_INTEGRATION=1` is set
 ...
 * OAK_INTEGRATION=1 \
 *   OAK_OPENAI_KEY=sk-... \
 *   OAK_ANTHROPIC_KEY=sk-ant-... \
 *   ./gradlew :composeApp:desktopTest --tests "*OakUiValidationTest*" --info
 * ```
 *
 ...
 * |--------------|-----------------------|--------------------------|------------------------------|
 * | OpenAI       | OAK_OPENAI_KEY        | OAK_OPENAI_MODEL         | gpt-4o-mini                  |
 * | Anthropic    | OAK_ANTHROPIC_KEY     | OAK_ANTHROPIC_MODEL      | claude-3-5-haiku-latest      |
 * | Gemini       | OAK_GEMINI_KEY        | OAK_GEMINI_MODEL         | gemini-2.0-flash             |
 * | Mistral      | OAK_MISTRAL_KEY       | OAK_MISTRAL_MODEL        | mistral-small-latest         |
 * | Groq         | OAK_GROQ_KEY          | OAK_GROQ_MODEL           | llama-3.3-70b-versatile      |
 * | OpenRouter   | OAK_OPENROUTER_KEY    | OAK_OPENROUTER_MODEL     | openai/gpt-4o-mini           |
 * | DeepSeek     | OAK_DEEPSEEK_KEY      | OAK_DEEPSEEK_MODEL       | deepseek-chat                |
 * | xAI          | OAK_XAI_KEY           | OAK_XAI_MODEL            | grok-2-latest                |
 * | Cerebras     | OAK_CEREBRAS_KEY      | OAK_CEREBRAS_MODEL       | llama-3.3-70b                |
 * | Moonshot     | OAK_MOONSHOT_KEY      | OAK_MOONSHOT_MODEL       | moonshot-v1-8k               |
 * | Together     | OAK_TOGETHER_KEY      | OAK_TOGETHER_MODEL       | meta-llama/Llama-3.3-70B-Instruct-Turbo |
 * | Mistral FT   | OAK_MISTRAL_FT_KEY    | OAK_MISTRAL_FT_MODEL     | ft:open-mistral-7b:latest  |
 *
 * Additional knobs:
 * - `OAK_REPORT_DIR` — where to write the report (default: `build/reports/oakui-integration`)
 * - `OAK_MIN_SUCCESS_RATE` — 0.0-1.0, fail the test below this rate (default: 0.0, never fail)
 * - `OAK_REPLAY_STRICT` — set to `1` to make the replay test fail when any saved response
 ...
 * 4. When green, copy the now-passing payloads into `OakUiParserTest` as regressions
 */
class OakUiValidationTest {

    @Test
    fun `validate oak-ui output across providers`() {
        if (System.getenv("OAK_INTEGRATION") != "1") {
            println("[OakUiValidationTest] Skipped — set OAK_INTEGRATION=1 to run.")
            return
        }

        val providers = discoverProviders()
        if (providers.isEmpty()) {
            println("[OakUiValidationTest] Skipped — no provider API keys found in env.")
            println("    Set any of: OAK_OPENAI_KEY, OAK_ANTHROPIC_KEY, OAK_GEMINI_KEY, OAK_GROQ_KEY, OAK_OPENROUTER_KEY")
            return
        }

        val reportDir = File(System.getenv("OAK_REPORT_DIR") ?: "build/reports/oakui-integration")
        reportDir.mkdirs()
        val requests = Requests()
        val allResults = mutableListOf<PromptResult>()

        for (provider in providers) {
            println("\n[OakUiValidationTest] === ${provider.label} (${provider.modelId}) ===")
        ...
        val minRate = System.getenv("OAK_MIN_SUCCESS_RATE")?.toDoubleOrNull() ?: 0.0
        if (minRate > 0.0) {
     ...
     *
     * Skips silently if no saved responses are found. Set `OAK_REPLAY_STRICT=1` to make
    ...
    fun `replay saved responses`() {
        ...
        if (!reportDir.isDirectory) {
            println("[OakUiValidationTest] Replay skipped — no report directory at ${reportDir.absolutePath}")
            return
        }
        ...
        if (rawFiles.isEmpty()) {
            println("[OakUiValidationTest] Replay skipped — no .raw.txt files under ${reportDir.absolutePath}")
            println("    Run the online test first with OAK_INTEGRATION=1 and at least one provider key.")
            return
        }

        println("\n[OakUiValidationTest] Replaying ${rawFiles.size} saved responses from ${reportDir.absolutePath}")
        var success = 0
        var partial = 0
        var noUi = 0
        var parseError = 0
        ...
        for (file in rawFiles) {
            ...
            val ui = blocks.filterIsInstance<OakUiBlock>().size
            val err = blocks.filterIsInstance<OakUiError>().size
        ...
        println("\n[OakUiValidationTest] Replay summary: $success ok, $partial partial, $noUi no-ui, $parseError parse-error  (total ${rawFiles.size})")

        val strict = System.getenv("OAK_REPLAY_STRICT") == "1"
    ...
    private fun discoverProviders(): List<ProviderSpec> {
        ...
        env("OAK_OPENAI_KEY")?.let {
            out += ProviderSpec(
                "openai",
                "OpenAI",
                Service.OpenAI,
                env("OAK_OPENAI_MODEL") ?: "gpt-4o-mini",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("OAK_ANTHROPIC_KEY")?.let {
            out += ProviderSpec(
                "anthropic",
                "Anthropic",
                Service.Anthropic,
                env("OAK_ANTHROPIC_MODEL") ?: "claude-3-5-haiku-latest",
                it,
                ProviderSpec.Kind.ANTHROPIC,
            )
        }
        env("OAK_GEMINI_KEY")?.let {
            out += ProviderSpec(
                "gemini",
                "Gemini",
                Service.Gemini,
                env("OAK_GEMINI_MODEL") ?: "gemini-2.0-flash",
                it,
                ProviderSpec.Kind.GEMINI,
            )
        }
        env("OAK_MISTRAL_KEY")?.let {
            out += ProviderSpec(
                "mistral",
                "Mistral",
                Service.Mistral,
                env("OAK_MISTRAL_MODEL") ?: "mistral-small-latest",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("OAK_GROQ_KEY")?.let {
            out += ProviderSpec(
                "groq",
                "Groq",
                Service.Groq,
                env("OAK_GROQ_MODEL") ?: "llama-3.3-70b-versatile",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("OAK_OPENROUTER_KEY")?.let {
            out += ProviderSpec(
                "openrouter",
                "OpenRouter",
                Service.OpenRouter,
                env("OAK_OPENROUTER_MODEL") ?: "openai/gpt-4o-mini",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("OAK_DEEPSEEK_KEY")?.let {
            out += ProviderSpec(
                "deepseek",
                "DeepSeek",
                Service.DeepSeek,
                env("OAK_DEEPSEEK_MODEL") ?: "deepseek-chat",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("OAK_XAI_KEY")?.let {
            out += ProviderSpec(
                ...
                "xAI",
                Service.XAI,
                env("OAK_XAI_MODEL") ?: "grok-2-latest",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("OAK_CEREBRAS_KEY")?.let {
            out += ProviderSpec(
                "cerebras",
                "Cerebras",
                Service.Cerebras,
                env("OAK_CEREBRAS_MODEL") ?: "llama-3.3-70b",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("OAK_MOONSHOT_KEY")?.let {
            out += ProviderSpec(
                "moonshot",
                "Moonshot",
                Service.Moonshot,
                env("OAK_MOONSHOT_MODEL") ?: "moonshot-v1-8k",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("OAK_TOGETHER_KEY")?.let {
            out += ProviderSpec(
                "together",
                "Together",
                Service.Together,
                env("OAK_TOGETHER_MODEL") ?: "meta-llama/Llama-3.3-70B-Instruct-Turbo",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
        env("OAK_MISTRAL_FT_KEY")?.let {
            out += ProviderSpec(
                "mistral-ft",
                "Mistral Fine-tuned",
                Service.Mistral,
                env("OAK_MISTRAL_FT_MODEL") ?: "ft:open-mistral-7b:latest",
                it,
                ProviderSpec.Kind.OPENAI_COMPAT,
            )
        }
    ...
    private fun runPrompt(
        ...
        val blocks = parseMarkdown(raw).blocks
        val uiBlocks = blocks.filterIsInstance<OakUiBlock>()
        val errorBlocks = blocks.filterIsInstance<OakUiError>()
    ...
    private fun writeReport(reportDir: File, results: List<PromptResult>, providers: List<ProviderSpec>) {
        val report = buildString {
            appendLine("# oak-ui Integration Report")
            appendLine()
        ...
        println("\n[OakUiValidationTest] Report written to: ${File(reportDir, "report.md").absolutePath}")
    }

    private fun printSummary(results: List<PromptResult>, providers: List<ProviderSpec>) {
        println("\n[OakUiValidationTest] === SUMMARY ===")
        for (p in providers) {
            val r = results.filter { it.provider.slug == p.slug }
    ...
    /**
     * Duplicates the oak-ui portion of the app's system prompt. Mirrors
    ...
    companion object {
        private val TEST_PROMPTS = listOf(
            TestPrompt(
                "simple-question",
                "Simple question expecting a text response with oak-ui elements",
            ...
            TestPrompt(
                "mixed-markdown",
                "Response that mixes markdown explanation with oak-ui interactive block",
