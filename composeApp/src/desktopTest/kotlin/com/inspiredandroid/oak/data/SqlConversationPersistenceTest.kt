...
class SqlConversationPersistenceTest {
    ...
    private fun createPersistence(settings: MapSettings = MapSettings()): SqlConversationPersistence {
        val driver = JdbcSqliteDriver(url = JdbcSqliteDriver.IN_MEMORY, schema = OakDatabase.Schema)
        return SqlConversationPersistence(OakDatabase(driver), AppSettings(settings))
    }
