...
class SshConfigManagerTest {
    ...
    fun ensureDefaultsCreatesBlockOnce() {
        ...
        assertContains(first, "# oak:defaults:start")
        ...
        assertContains(first, "# oak:defaults:end")
    ...
    fun upsertHostAddsBlockAndSeedsDefaults() {
        ...
        assertTrue(changed)
        val text = configText()
        assertContains(text, "# oak:defaults:start")
        assertContains(text, "# oak:host:prod:start")
        assertContains(text, "Host prod")
        assertContains(text, "HostName 1.2.3.4")
        assertContains(text, "# oak:host:prod:end")
    }
    ...
    fun upsertHostReplacesPreviousBlock() {
        ...
        val startCount = "# oak:host:prod:start".toRegex().findAll(text).count()
        val endCount = "# oak:host:prod:end".toRegex().findAll(text).count()
