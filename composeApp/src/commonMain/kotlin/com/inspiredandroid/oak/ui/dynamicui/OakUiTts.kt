...
import com.inspiredandroid.oak.ui.markdown.toSpeakableText

/**
 * TTS-friendly text for a message that may contain oak-ui fences. Routed through the unified
 * markdown parser: formatting is stripped, code blocks dropped, and oak-ui blocks walked for
...
internal fun OakUiNode.collectSpeakableText(): String {
    val parts = mutableListOf<String>()
    walk(parts)
    return parts.asSequence().filter { it.isNotBlank() }.joinToString(". ")
}

private fun OakUiNode.walk(parts: MutableList<String>) {
    when (this) {
