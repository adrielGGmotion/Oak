...
/**
 * Decodes the body of a `oak-ui` fenced JSON block into a [OakUiNode].
 *
 * The parse pipeline runs each block through three stages:
 *   1. **Block extraction** — the markdown parser locates `oak-ui` fences and hands the raw
 ...
 *      [parseNode] in `OakUiNodeBuilders.kt`, constructing [OakUiNode] instances field-by-field.
...
object OakUiParser {
    ...
    sealed interface UiBlockResult {
        data class Ui(val node: OakUiNode, val rawJson: String) : UiBlockResult
        data class Error(val rawJson: String) : UiBlockResult
    }

    /**
     * Decode the raw body of a oak-ui fence (everything between the opening and closing triple
     * backticks). Returns either a decoded [OakUiNode] or an [UiBlockResult.Error] carrying the
    ...
    fun parseUiBlockBody(rawBlock: String): UiBlockResult? {
        ...
        return try {
            ...
            println("oak-ui parse error: ${e.message} | ${json.take(500)}")
    ...
    private fun tryParseLine(line: String): OakUiNode? = runCatching { parseSingleNode(line) }.getOrNull()
        ...
        ?: run {
            println("oak-ui parse error: failed to deserialize line | ${line.take(500)}")
            null
        }
