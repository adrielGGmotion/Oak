...
/**
 * Oak Build's Debian-under-proot environment. Separate from [SandboxController]
 * on purpose: different rootfs, different home, and resetting one must never
 * touch the other.
 */
interface OakBuildController {
    val state: StateFlow<OakBuildState>

    /** The Debian file tree, for the Files tab. Empty on platforms without Oak Build. */
...
/** Oak Build is Android-only; every other target gets this. */
class NoOpOakBuildController : OakBuildController {
    override val state = MutableStateFlow(OakBuildState())
...
expect fun createOakBuildController(): OakBuildController
