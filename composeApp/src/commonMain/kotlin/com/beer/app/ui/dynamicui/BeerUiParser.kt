...
/**
 * Decodes the body of a `beer-ui` fenced JSON block into a [BeerUiNode].
 *
 * The parse pipeline runs each block through three stages:
 *   1. **Block extraction** — the markdown parser locates `beer-ui` fences and hands the raw
 ...
 *      [parseNode] in `BeerUiNodeBuilders.kt`, constructing [BeerUiNode] instances field-by-field.
...
object BeerUiParser {
    ...
    /** Result of decoding a beer-ui fence body; consumed by the markdown parser. */
    sealed interface UiBlockResult {
        data class Ui(val node: BeerUiNode, val rawJson: String) : UiBlockResult
        data class Error(val rawJson: String) : UiBlockResult
    }

    /**
     * Decode the raw body of a beer-ui fence (everything between the opening and closing triple
     * backticks). Returns either a decoded [BeerUiNode] or an [UiBlockResult.Error] carrying the
    ...
    fun parseUiBlockBody(rawBlock: String): UiBlockResult? {
        ...
        return try {
            ...
            println("beer-ui parse error: ${e.message} | ${json.take(500)}")
    ...
    private fun tryParseLine(line: String): BeerUiNode? = runCatching { parseSingleNode(line) }.getOrNull()
        ?: runCatching { parseSingleNode(sanitizeJson(line)) }.getOrNull()
        ?: run {
            println("beer-ui parse error: failed to deserialize line | ${line.take(500)}")
            null
        }

    /** Parse a repaired JSON string into a [BeerUiNode] via the direct builder pipeline. */
    private fun parseSingleNode(json: String): BeerUiNode? = parseNode(SharedJson.parseToJsonElement(json))
