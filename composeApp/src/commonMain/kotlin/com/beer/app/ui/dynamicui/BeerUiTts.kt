...
/**
 * TTS-friendly text for a message that may contain beer-ui fences. Routed through the unified
 * markdown parser: formatting is stripped, code blocks dropped, and beer-ui blocks walked for
...
internal fun BeerUiNode.collectSpeakableText(): String {
    val parts = mutableListOf<String>()
    walk(parts)
    return parts.asSequence().filter { it.isNotBlank() }.joinToString(". ")
}

private fun BeerUiNode.walk(parts: MutableList<String>) {
    when (this) {
