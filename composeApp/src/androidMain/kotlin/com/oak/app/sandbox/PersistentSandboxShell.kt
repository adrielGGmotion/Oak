...
private const val PID_PROBE_PREFIX = "${RS}BEERBASHPID$US"
...
class PersistentSandboxShell(
    ...
    suspend fun run(
        ...
        val cmdFile = File(tmpPath, ".oak_cmd_$nonce")
        try {
        ...
        val line = ". /tmp/.oak_cmd_$nonce; __oak_st=\$?; rm -f /tmp/.oak_cmd_$nonce; " +
            "printf '\\n\\036%s\\037%d\\037%d\\037%s\\036\\n' '$nonce' \"\$__oak_st\" \"\$\$\" \"\$PWD\" >&2"
    ...
    private fun ensureShell() {
        ...
        val h = executor.executeStreaming(
            command = "ulimit -t 120 -v 268435456 -u 100 && exec bash --noprofile --norc",
