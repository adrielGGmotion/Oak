...
/** Renames legacy kai_secure_prefs → oak_secure_prefs on disk, if the new file
...
private fun migrateSecurePrefsFile(context: Context) {
    ...
    val newFile = java.io.File(prefsDir, "oak_secure_prefs.xml")
    if (oldFile.exists() && !newFile.exists() && !oldFile.renameTo(newFile)) {
        throw java.io.IOException("Failed to migrate kai_secure_prefs.xml → oak_secure_prefs.xml")
    }
}
...
actual fun createLegacySettings(): Settings? {
    val context: Context by inject(Context::class.java)
    val prefs = context.getSharedPreferences("com.oak.app_preferences", Context.MODE_PRIVATE)
