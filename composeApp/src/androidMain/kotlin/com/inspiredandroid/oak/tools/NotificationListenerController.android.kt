...
private const val NOTIFICATION_LISTENER_FQN =
    "com.inspiredandroid.oak.notifications.OakNotificationListenerService"
...
actual class NotificationListenerController actual constructor() {
    ...
    actual fun openAccessSettings() {
        if (!supported) return
        // Try the per-component variant on API 30+ first — drops the user directly on Oak's row.
